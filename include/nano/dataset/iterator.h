...
namespace nano
{
    ...
    public:
        ...
        feature_iterator_t(data_cmap_t data, mask_cmap_t mask, indices_cmap_t samples,
            tensor_size_t index = 0, indices_cmap_t shuffled = indices_cmap_t{}) :
            m_index(index),
            m_data(data),
            m_mask(mask),
            m_samples(samples),
            m_shuffled(shuffled)
        {
            ...
            assert(shuffled.size() == 0 || shuffled.size() == data.template size<0>());
        }
        ...
        auto operator*() const
        {
            ...
            const auto sample = (m_shuffled.size() == 0) ? m_samples(m_index) : m_shuffled(m_samples(m_index));
    ...
    private:
        ...
        indices_cmap_t      m_shuffled; ///< optional shuffled indices
    };
    ...
    auto make_iterator(const tensor_t<tstorage, tscalar, trank>& data, mask_cmap_t mask, indices_cmap_t samples,
        indices_cmap_t shuffled = indices_cmap_t{})
    {
        return feature_iterator_t<tscalar, trank>{data, mask, samples, 0, shuffled};
    }
    ...
    auto make_end_iterator(const tensor_t<tstorage, tscalar, trank>& data, mask_cmap_t mask, indices_cmap_t samples,
        indices_cmap_t shuffled = indices_cmap_t{})
    {
        return feature_iterator_t<tscalar, trank>{data, mask, samples, samples.size(), shuffled};
    }
}

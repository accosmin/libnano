...
UTEST_BEGIN_MODULE(test_dataset_cifar100f)

UTEST_CASE(load)
{
    const auto dataset = imclass_dataset_t::all().get("cifar100f");
    ...
    UTEST_CHECK_EQUAL(dataset->target().labels().size(), 100U);

    UTEST_CHECK_EQUAL(dataset->idims(), make_dims(32, 32, 3));
    UTEST_CHECK_EQUAL(dataset->tdims(), make_dims(100, 1, 1));
